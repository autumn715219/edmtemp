...
const redo = () => (dispatch, getState) => {
  ...
  if (index < undoStack.length - 1) {
    ...
    dispatch(setPageList(nextPageInfo.pageList));
    dispatch(setCurrentSelectPage(nextPageInfo.currentSelectPage));
...
export { addUndoStack, setUndoStack, initUndoStack, clearUndoStack, undo, redo };
